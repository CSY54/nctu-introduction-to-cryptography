...
const std::string DEBUG_KEY = "security";
const std::string DEBUG_PLAINTEXT = "Hi Mary!";
const std::string DEBUG_CIPHERTEXT = "303B1E1CBA103695";
...
int main() {
	...
	DES sample_des(DES::string_to_int(DEBUG_KEY));
	uint64_t sample_ciphertext = sample_des.encrypt(DES::string_to_int(DEBUG_PLAINTEXT));
	assert(DES::int_to_hex(sample_ciphertext) == DEBUG_CIPHERTEXT);
	uint64_t sample_plaintext = sample_des.decrypt(DES::hex_to_int(DEBUG_CIPHERTEXT));
	assert(DES::int_to_string(sample_plaintext) == DEBUG_PLAINTEXT);

#else

	std::ifstream fin("./DES-Key-Ciphertext.txt");
	std::ofstream fout("./des-out.txt");

	std::string key;
	std::string ciphertext;
	while (fin >> key >> ciphertext) {
		DES des(DES::string_to_int(key));

		uint64_t plaintext = des.decrypt(DES::hex_to_int(ciphertext));

		fout << DES::int_to_string(plaintext) << std::endl;
	}
